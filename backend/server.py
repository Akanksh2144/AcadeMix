...
from fastapi import FastAPI, HTTPException, Request, Response, Depends, Query, UploadFile, File, Form, Body
...
async def review_leave(leave_id: str, req: LeaveReview, user: dict = Depends(get_current_user), session: AsyncSession = Depends(get_db)):
    leave_r = await session.execute(
        select(models.LeaveRequest).where(
            models.LeaveRequest.id == leave_id,
            models.LeaveRequest.college_id == user["college_id"]
    ...
    # Leave Routing Validation Constraint
...
async def get_institution_profile(user: dict = Depends(require_role("admin", "principal")), session: AsyncSession = Depends(get_db)):
    result = await session.execute(
...
async def update_institution_profile(req: InstitutionProfileUpdate, user: dict = Depends(require_role("admin", "principal")), session: AsyncSession = Depends(get_db)):
    result = await session.execute(
        select(models.InstitutionProfile).where(
            models.InstitutionProfile.college_id == user["college_id"]
        )
...
@app.post("/api/admin/alumni/batch-graduate")
...
@app.get("/api/admin/alumni/pending")
...
@app.put("/api/admin/alumni/{alumni_id}/verify")
...
@app.post("/api/admin/alumni/contributions")
...
@app.get("/api/admin/reports/alumni-outcomes")
async def get_alumni_outcomes(
    user: dict = Depends(require_role("admin", "nodal_officer", "super_admin")),
    session: AsyncSession = Depends(get_db)
):
    # Total alumni
    alumni_count = (await session.execute(
        select(func.count(models.User.id)).where(models.User.college_id==user["college_id"], models.User.role=="alumni")
    )).scalar()
    
    # Total contributions sum
    contrib_sum = (await session.execute(
        select(func.sum(models.AlumniContribution.amount)).where(models.AlumniContribution.college_id==user["college_id"])
    )).scalar() or 0
    
    # Active mentorships
    mentor_req_count = (await session.execute(
        select(func.count(models.AlumniMentorship.id)).where(models.AlumniMentorship.college_id==user["college_id"])
    )).scalar()
...
@app.post("/api/admin/alumni-events")
...
@app.put("/api/admin/alumni-events/{event_id}")
...
@app.put("/api/admin/alumni-events/{event_id}/attendance")
...
@app.put("/api/admin/alumni/achievements/{achievement_id}/verify")
...
@app.put("/api/tpo/alumni-jobs/{job_id}/moderate")
...
@app.post("/api/alumni/job-postings")
...
@app.get("/api/alumni/job-postings")
...
@app.post("/api/alumni/mentorship/{id}/respond")
...
@app.post("/api/alumni/mentorship/{id}/session-note")
...
@app.post("/api/alumni/events/{event_id}/register")
...
@app.post("/api/alumni/achievements")
...
@app.put("/api/alumni/progression/higher-studies")
...
# ─── Student Endpoints (Consuming Alumni Service) ──────────────────

@app.get("/api/student/alumni-jobs")
...
@app.get("/api/student/alumni-mentors")
async def browse_available_mentors(
    ...
    alumni = (await session.execute(stmt)).all()
...
@app.post("/api/student/alumni-mentorship/request")
...
@app.post("/api/industry/curriculum-feedback")
...
@app.post("/api/industry/employer-feedback")
...
@app.post("/api/industry/projects")
...
@app.post("/api/industry/training-programs")
...
# ==============================================================================
# RETIRED FACULTY MODULE ENDPOINTS
# ==============================================================================
...
@app.post("/api/admin/retired-faculty/{user_id}/advisory-roles")
...
@app.get("/api/admin/retired-faculty/available-lecturers")
...
@app.get("/api/admin/retired-faculty/{user_id}/entitlements")
...
@app.put("/api/admin/retired-faculty/{user_id}/entitlements")
...
@app.get("/api/admin/reports/retired-faculty-research")
async def get_retired_faculty_research_report(
    ...
    result = await session.execute(
        select(models.RetiredFacultyResearch, models.User).join(
            ...
            models.RetiredFacultyResearch.college_id == user["college_id"],
...
@app.get("/api/admin/reports/consultancy")
async def get_consultancy_report(
    ...
    result = await session.execute(
        select(models.ConsultancyEngagement, models.User).join(
            ...
            models.ConsultancyEngagement.college_id == user["college_id"],
...
@app.get("/api/retired-faculty/dashboard")
...
@app.get("/api/retired-faculty/my-roles")
...
@app.get("/api/retired-faculty/research")
...
@app.post("/api/retired-faculty/research")
...
@app.get("/api/retired-faculty/consultancy")
...
@app.post("/api/retired-faculty/consultancy")
...
@app.get("/api/retired-faculty/my-entitlements")
...
@app.post("/api/retired-faculty/events/{event_id}/register")
